...
[ONNX](https://onnx.ai/) and OpenVINO export and inference for [SAM3](https://github.com/facebookresearch/sam3).
...
# download pre-exported openvino model https://intel.sharepoint.com/:f:/s/gpuclientai/IgDhDjV-4xC-TIBuMslTQrewAf9_Mb-y56bmPKRpdZSoiAM?e=2qFRfC
uv run --no-dev infer_openvino.py --image images/bus.jpg --text-prompt person --device CPU --model-folder models
uv run --no-dev infer_openvino.py --image images/sofa.jpg --text-prompt sofa --device CPU --model-folder models
uv run --no-dev infer_openvino.py --image images/dog.jpg --box-prompt --device CPU --model-folder models  # opens a gui to select box
...
# With box prompt
uv run infer_onnx.py --image images/bus.jpg --box-prompt 0.548,0.658,0.043,0.095
uv run infer_onnx.py --image images/bus.jpg --box-prompt  # opens a gui to select box
```
...
```sh
# With text prompt (CPU)
uv run infer_openvino.py --image images/bus.jpg --text-prompt person --device CPU --model-folder models

# With box prompt
uv run infer_openvino.py --image images/bus.jpg --box-prompt 0.548,0.658,0.043,0.095 --device CPU --model-folder models
uv run infer_openvino.py --image images/bus.jpg --box-prompt --device CPU --model-folder models  # opens a gui to select box
